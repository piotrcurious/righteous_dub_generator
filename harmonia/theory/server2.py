...
Implemented fixes (2026-02-26):
 - Correct modular distance helpers and use them consistently.
 - Fix target selection in plr_transform (modular signed distance).
 - Replace factorial orbifold matching with exact assignment using
   a pure-Python Hungarian (Munkres) algorithm (optimal for any n).
 - Fallback/performance-safe behavior preserved for small/large chords.
 - Triad completion message formatting fixed.
 - tonnetz_projection_interval: improved exact p/q mapping search:
   searches lattice coordinates (fifths, thirds) for exact modular matches;
   otherwise returns best-fit coordinates minimizing lattice distance.
...
def mod_signed_dist(a: int, b: int, modulus: int = 12) -> int:
    """Return signed modular distance from a -> b in range [-modulus//2 .. +modulus//2]."""
    d = (b - a) % modulus
    if d > modulus // 2:
        d -= modulus
    return d
...
def prime_form(pcs: List[int]) -> Tuple:
    ...
    def normal_form(rotation):
        base = rotation[0]
        t = tuple((p - base) % 12 for p in rotation)
        return t
    ...
    def pack_score(c):
        # Primary key: span (last element). Secondary key: lexicographic.
        return (c[-1], c)

    return min(candidates, key=pack_score)


# Forte set-class table (partial)
...
def set_class_info(pcs: List[int]) -> Dict[str, Any]:
    ...
    z_related = None
    if "Z15" in forte_name:
        z_related = "4-Z29"
    if "Z29" in forte_name:
        z_related = "4-Z15"
...
def triad_pcs(root: int, quality: str) -> List[int]:
    ...
    if q in ('dim', 'diminished'):
        return [root % 12, (root + 3) % 12, (root + 6) % 12]
    if q in ('aug', 'augmented'):
        return [root % 12, (root + 4) % 12, (root + 8) % 12]
    return [root % 12, (root + 4) % 12, (root + 7) % 12]
...
def plr_transform(root: int, quality: str, op: str) -> Dict[str, Any]:
    ...
    for cf in changed_from:
        if changed_to:
            # choose target that minimizes modular absolute distance
            ct = min(changed_to, key=lambda x: mod_abs_dist(cf, x, 12))
            diff = mod_signed_dist(cf, ct, 12)
...
def plr_path(root: int, quality: str, target_root: int, target_quality: str,
    ...
    start = (root, quality)
    goal = (target_root, target_quality)
    if start == goal:
        return []
    queue = deque([(start, [])])
    visited = {start}
...
# Hungarian (Munkres) algorithm implementation for assignment (pure Python).
# Returns row_ind, col_ind for minimal total cost assignment on a square cost matrix.
def linear_sum_assignment(cost_matrix: List[List[float]]) -> Tuple[List[int], List[int]]:
    """Pure-Python implementation of the Hungarian algorithm (works for n x n)."""
    # This implementation follows standard algorithm structure. Complexity O(n^3).
    n = len(cost_matrix)
    if n == 0:
        return [], []
    # Convert to float
    ...
    # p[j] is assigned row for column j
    row_ind = [-1] * n
    col_ind = [-1] * n
    for j in range(1, n + 1):
        if p[j] != 0:
    ...
    assigned_cols = [row_ind[i] for i in range(n)]
    assigned_rows = list(range(n))
    return assigned_rows, assigned_cols


def orbifold_voice_leading(chord_a: List[int], chord_b: List[int]) -> Dict[str, Any]:
    """Optimal voice-leading in T^n/S_n using assignment; exact for any n (Hungarian)."""
    ...
    # Pad shorter lists by duplicating last element (deterministic)
    if len(a) < n:
        a = list(a) + [a[-1]] * (n - len(a))
    if len(b) < n:
        b = list(b) + [b[-1]] * (n - len(b))

    # cost matrix: absolute modular distances
    cost = [[mod_abs_dist(ai, bj, 12) for bj in b] for ai in a]

    # Use Hungarian (pure-Python) for exact optimal assignment
    try:
        rows, cols = linear_sum_assignment(cost)
        # rows = list(range(n)), cols = assigned column for each row
        pairs = [(i, cols[i]) for i in range(n)]
    except Exception:
        # As a robust fallback (shouldn't occur), use greedy
        pairs = []
        assigned_b = set()
        for i in range(n):
            candidates = sorted(((cost[i][j], j) for j in range(n) if j not in assigned_b), key=lambda x: x[0])
            if not candidates:
                continue
            _, j = candidates[0]
            assigned_b.add(j)
    ...
    match = []
    for i, j in pairs:
        diff = mod_signed_dist(a[i], b[j], 12)
    ...
    total_distance = sum(abs(m["semitones"]) for m in motions)
    smoothness = "smooth" if total_distance <= 3 else "moderate" if total_distance <= 6 else "disjunct"

    return {
        "distance": total_distance, "motions": motions,
        "contrary_motions": n_contrary, "parallel_motions": n_parallel,
        "oblique_motions": n_oblique, "smoothness": smoothness,
        "description": f"Min. {total_distance} semitone{'s' if total_distance != 1 else ''} — {smoothness}",
    }
    ...
    for op in ['P', 'L', 'R']:
        ...
        results.append({
            ...
            "explanation": (
                ...
                f"Single-voice motion: "
                + (f"{t['voice_motions'][0]['from_name']}->{t['voice_motions'][0]['to_name']}"
...
def tonnetz_projection_interval(pc: int, edo: int = 12) -> Tuple[int, int]:
    ...
    Approach:
     - Compute EDO step counts for just fifth (3/2) and just third (5/4)
       as exact rational approximations in steps: steps_fifth = round(edo * log2(3/2))
       and steps_third = round(edo * log2(5/4)).
     - Search for integer lattice coordinates (a,b) within a reasonable range
       for which the congruence (a*steps_fifth + b*steps_third) % edo == pc % edo
       holds exactly (this is the "exact p/q mapping" attempt).
     - If no exact congruence solution is found in the search bounds,
       return the (a,b) that minimizes weighted lattice distance while minimizing
       modular mismatch (closest residue).
    """
    # exact step counts for generators (as floats then rounded to integer steps)
    steps_fifth = round(edo * math.log2(3 / 2))
    steps_third = round(edo * math.log2(5 / 4))

    target = pc % edo

    # search ranges (expanded to find exact congruence when possible)
    # These ranges are conservative but broad enough for typical EDO values.
    a_range = range(-edo, edo + 1)
    b_range = range(-edo // 2, edo // 2 + 1)
    ...
    for a in a_range:
        for b in b_range:
            ...
            # weight lattice distance (Manhattan with b scaled) and mismatch
            lattice_dist = abs(a) + abs(b) * 1.4
            # Prefer exact matches (mismatch == 0) strongly
            score = (0 if mismatch == 0 else 1000) + lattice_dist + mismatch * 0.01
            if score < best_score:
                best_score = score
                best = (a, b)
                best_mismatch = mismatch
                if mismatch == 0:
                    # perfect congruence found; we can break inner loops if desired
                    pass
    return best
...
def ji_ratio(fifths: int, thirds: int, sevenths: int = 0) -> Tuple[int, int, float]:
    # compute fractional exponent of 2 from primes and convert to cents
    log2_val = (fifths * math.log2(3) + thirds * math.log2(5) +
                sevenths * math.log2(7))
    cents = (log2_val % 1.0) * 1200.0
    # rational approximation for numerator/denominator using Fraction
    # attempt to build a ratio from prime exponents
    num = 3 ** max(0, fifths) * 5 ** max(0, thirds) * 7 ** max(0, sevenths)
    den = 3 ** max(0, -fifths) * 5 ** max(0, -thirds) * 7 ** max(0, -sevenths)
    # reduce by power-of-two normalization so ratio within [1,2)
    while num >= 2 * den:
        den *= 2
    while den > num:
        num *= 2
...
def suggest_completion(pitch_classes: List[int], key: int, edo: int = 12) -> List[Dict[str, Any]]:
    ...
    for pc in range(12):
        if pc in existing:
            continue
        roughness_delta = sum(
            INTERVAL_ROUGHNESS.get(mod_abs_dist(pc, e, 12), 0.3)
            for e in existing)
        ...
        if triad_id:
            r, q = triad_id
            qual_label = "maj" if q == 'maj' else "min" if q == 'min' else q
            reasons.append(f"Completes {nn(r)} {qual_label} triad")
        lt = (key + 11) % 12
        sd = (key + 5) % 12
        if pc == lt:
            reasons.append(f"Adds leading tone {nn(lt)} (7) -> dominant function")
