...
private:
    ...
    // Instrument Builder
    std::unique_ptr<InstrumentWindow> instrument_win_;
    Fl_Button* btn_open_instrument_{nullptr};
    Fl_Button* btn_capture_chord_{nullptr};
    ...
    static void cbOpenInstrument(Fl_Widget*, void* d);
    static void cbCaptureChord(Fl_Widget*, void* d);

    void playChord(const InstrumentChord& chord, bool sustain);
    void releaseChord(const InstrumentChord& chord);
...
void HarmoniaApp::buildUI() {
    ...
    // ── SIDE PANEL (right) ────────────────────────────────────────────────────
    {
        ...
        py = content_y + 60;
        btn_open_instrument_ = new Fl_Button(px + 20, py, pw - 40, 40, "OPEN INSTRUMENT WINDOW"); py += 60;
        btn_capture_chord_ = new Fl_Button(px + 20, py, pw - 40, 30, "Capture to Selected Slot");
        grp_instrument_->end();
...
void HarmoniaApp::setupCallbacks() {
    ...
    btn_open_instrument_->callback(cbOpenInstrument, this);
    btn_capture_chord_->callback(cbCaptureChord, this);
...
void HarmoniaApp::playChord(const InstrumentChord& chord, bool sustain) {
...
void HarmoniaApp::releaseChord(const InstrumentChord& chord) {
    for(const auto& n : chord.notes) {
        auto voices = audio_->getVoiceSnapshot();
        for(auto& v : voices) {
            double ds = std::abs(std::log2(v.frequency / n.freq) * current_edo_);
...
void HarmoniaApp::cbBrowserResolutions(Fl_Widget* w, void* d) {
    ...
    int idx=(int)(intptr_t)b->data(l)-1; if(idx>=0 && idx<(int)a->last_resolutions_.size()){
        ...
        if(Fl::event_clicks()){
            InstrumentChord ck; ck.name=rp.target_label;
            for(int pc:rp.target_pcs) {
                double freq = C4_HZ*std::pow(2.0, (double)pc/a->current_edo_);
                ck.notes.push_back({pc, freq, 4});
            }
            a->playChord(ck, true);
            Fl::event_clicks(0);
        }
...
void HarmoniaApp::cbBrowserPivots(Fl_Widget* w, void* d) {
    ...
    int idx=(int)(intptr_t)b->data(l)-1; if(idx>=0 && idx<(int)a->last_pivot_res_.all_pivots.size()){
        ...
        if(Fl::event_clicks()){
            InstrumentChord ck; ck.name=p.label;
            for(int pc:p.pcs) {
                double freq = C4_HZ*std::pow(2.0, (double)pc/a->current_edo_);
                ck.notes.push_back({pc, freq, 4});
            }
...
void HarmoniaApp::cbOpenInstrument(Fl_Widget*, void* d) { ((HarmoniaApp*)d)->instrument_win_->show(); }
void HarmoniaApp::cbCaptureChord(Fl_Widget*, void* d) {
    ...
    const char* slot_s = fl_input("Enter slot (0-9):", "0");
    if (slot_s) {
        int slot = atoi(slot_s);
        a->instrument_win_->setChord(slot, ck);
    }
}
