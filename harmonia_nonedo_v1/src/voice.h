...
enum class TuningMode { EDO, LATTICE };
...
struct Voice {
    ...
    // ── pitch
    TuningMode mode{TuningMode::EDO};
    ...
    // Lattice coords (for Rank-r Lattice Temperament)
    ...
    Voice(const Voice& o)
        : name(o.name), id(o.id), mode(o.mode), frequency(o.frequency), pitch_class(o.pitch_class),
          octave(o.octave), edo(o.edo), rank(o.rank), detune_cents(o.detune_cents), amplitude(o.amplitude),
        ...
        for(int i=0; i<4; ++i) lattice_coords[i] = o.lattice_coords[i];
    ...
    Voice(Voice&& o) noexcept
        : name(std::move(o.name)), id(o.id), mode(o.mode), frequency(o.frequency),
          pitch_class(o.pitch_class), octave(o.octave), edo(o.edo), rank(o.rank), detune_cents(o.detune_cents),
    ...
    void setFrequency(double freq, bool update_coords = true) {
        ...
        if (mode == TuningMode::EDO) {
            ...
            // In lattice mode, pitch_class/octave might be less relevant
            // but we can still map to nearest 12-EDO for default display/color
            double total_steps = std::round(std::log2(f_actual / C4_HZ) * 12.0);
            pitch_class = (int)std::fmod(total_steps, 12.0);
            if (pitch_class < 0) pitch_class += 12;
            octave = 4 + (int)std::floor(total_steps / 12.0);
        }
