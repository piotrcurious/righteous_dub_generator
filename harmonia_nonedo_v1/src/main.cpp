...
private:
    ...
    static void cbMode(Fl_Widget*, void* d);
    ...
    void onTonalSpaceClick(int pc, int oct, double freq);
...
void HarmoniaApp::buildUI() {
    ...
    // ── TOOLBAR ──────────────────────────────────────────────────────────────
    {
        ...
        new Fl_Box(x,y,40,bh,"Mode:"); x+=40;
        ch_mode_ = new Fl_Choice(x,y,80,bh); ch_mode_->add("EDO|Lattice"); ch_mode_->value(0); x += 86;
...
void HarmoniaApp::setupCallbacks() {
    ...
    tonal_space_->setNodeClickCallback([this](int pc, int oct, double freq){ onTonalSpaceClick(pc, oct, freq); });
}
...
void HarmoniaApp::onIdle(void* data) {
    ...
    for(auto& v : voices) {
        if (auto* s = app->findStrip(v.id)) {
            ...
            int display_edo = (v.mode == TuningMode::EDO) ? app->current_edo_ : 12;
            s->lbl_note->copy_label(noteName(v.pitch_class, v.octave, display_edo).c_str());
            if (v.env_stage == Voice::EnvStage::IDLE && !v.note_on.load() && !s->manual) { app->removeVoice(v.id); break; }
        }
    }
}

void HarmoniaApp::onTonalSpaceClick(int pc, int oct, double freq) {
    (void)pc; (void)oct;
    auto voices = audio_->getVoiceSnapshot(); int eid = -1;
    double total_steps_edo = std::round(std::log2(freq / C4_HZ) * current_edo_);

    for (auto& v : voices) {
        if (audio_->getTuningMode() == TuningMode::EDO) {
            double v_total_steps = std::round(std::log2(v.frequency / C4_HZ) * current_edo_);
            if ((int)v_total_steps == (int)total_steps_edo) { eid = v.id; break; }
        }
...
void HarmoniaApp::cbMode(Fl_Widget* w, void* d) {
    auto* a = (HarmoniaApp*)d;
    int m = ((Fl_Choice*)w)->value();
    if (m == 0) { // EDO
...
void HarmoniaApp::cbBrowserFunction(Fl_Widget* w, void* d) {
    ...
    if(Fl::event_clicks() && v>=1000) {
        double freq = C4_HZ * std::pow(2.0, (double)(v-1000) / a->current_edo_);
        a->onTonalSpaceClick(v-1000, 4, freq);
        Fl::event_clicks(0);
    }
}
