...
void AudioEngine::setEDO(int edo) {
    edo_.store(edo);
    mode_.store(TuningMode::EDO);
    std::lock_guard<std::mutex> lk(voices_mutex_);
    for (auto& v : voices_) {
        v.mode = TuningMode::EDO;
        v.edo = edo;
...
void AudioEngine::setLatticeTuning(const std::vector<double>& generators, const std::vector<int>& primes) {
    ...
    std::lock_guard<std::mutex> lk(voices_mutex_);
    for (auto& v : voices_) {
        v.mode = TuningMode::LATTICE;
        v.rank = (int)generators.size();
        // In lattice mode, we might not want to snap existing voices
        // to a grid automatically unless they were created on one.
        v.setFrequency(v.frequency, false);
        Voice::pcColorHSV(v.pitch_class, 12, v.color[0], v.color[1], v.color[2]);
        v.name = noteName(v.pitch_class, v.octave, 12);
    }
}
