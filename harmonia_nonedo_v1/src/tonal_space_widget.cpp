    ...
    if (mode_ == TuningMode::EDO) {
        ...
        for (int oct = 2; oct <= 6; oct++) {
            for (int pc = 0; pc < edo_; pc++) {
                ...
                n.cx = n.cy = 0.f;
                n.wx = n.radius * std::cos(n.angle);
                n.wy = n.radius * std::sin(n.angle);
...
void TonalSpaceWidget::updateNodeCoordinates() {
    for (auto& n : nodes_) {
        if (mode_ == TuningMode::EDO) {
            ...
            // Lattice mode: simple translation/rotation of world coords
            float rx = n.wx * std::cos(rotation_) - n.wy * std::sin(rotation_);
            float ry = n.wx * std::sin(rotation_) + n.wy * std::cos(rotation_);
            worldToScreen(rx, ry, n.cx, n.cy);
        }
    }
}
...
void TonalSpaceWidget::drawNodes() {
    float threshold = (mode_ == TuningMode::EDO) ? (0.5f / edo_) : 0.001f;
    ...
    for (auto& n : nodes_) {
        ...
        float pr, pg, pb;
        Voice::pcColorHSV(n.pitch_class, (mode_ == TuningMode::EDO ? edo_ : 12), pr, pg, pb);
...
void TonalSpaceWidget::drawLabels() {
    ...
    for (auto& n : nodes_) {
        ...
        for (auto& v : voices_) {
            if (v.active && v.frequency > 0.f && n.freq > 0.f) {
                float ratio = std::abs((float)std::log2(v.frequency / n.freq));
                if (ratio < threshold) { has_voice = true; break; }
            }
        }
...
int TonalSpaceWidget::handle(int event) {
    ...
    case FL_PUSH:
        ...
        if (Fl::event_button() == FL_LEFT_MOUSE) {
            ...
            if (n && node_click_cb_) {
                node_click_cb_(n->pitch_class, n->octave, n->freq);
            }
        }
        return 1;
    case FL_DRAG:
