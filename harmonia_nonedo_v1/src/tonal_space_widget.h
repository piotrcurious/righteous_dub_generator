...
struct TonalNode {
    int pitch_class;   // 0 to EDO-1 or nearest 12
    int octave;        // 0-8
    int lattice_coords[4]{0,0,0,0};
    int rank{0};
    float angle;       // radians (circular)
    float radius;      // distance from center (circular)
    float wx, wy;      // world coords
    float cx, cy;      // screen coords (updated each draw)
    float freq;        // expected frequency (Hz)
    std::string label;
};
