...
MODULE 0  PSYCHOACOUSTIC ENGINE
   ...
   0.11 Intermodulation solver  (CDT/QDT combination tones; ICI scoring)
...
def auditory_cortex_model(
    ...
    im_data        = chord_intermodulation_analysis(
        pcs, c4_hz, octave, n_harmonics, level_db=level_db, edo=edo)
    ...
    # ── Combined perceptual tension  (four-component model) ────────────────
    # Weights updated to include intermodulation roughness (ICI-derived):
    #   0.35 · roughness(Sethares) + 0.25 · (1−harmonicity) +
    #   0.25 · (1−tonalStability) + 0.15 · im_roughness_normalized
    # The IM component captures hidden dissonance from combination tones that
    # the Sethares model misses (beating of phantom partials against primaries).
    t_rough  = roughness_data["roughness_normalized"]
    t_harm   = 1.0 - vp_data["harmonicity"]
    t_tonal  = 1.0 - kk_data["kk_normalized"]
    t_im     = im_data["im_roughness_normalized"]          # 0–1 from IM solver
    p_tension = 0.35 * t_rough + 0.25 * t_harm + 0.25 * t_tonal + 0.15 * t_im
    ...
    return {
        "level_1_peripheral": {
            ...
            # Intermodulation sub-block
            "intermodulation": {
                "ici"                    : im_data["ici"],
                "ici_label"              : im_data["ici_label"],
                "im_roughness"           : im_data["im_roughness"],
                "im_roughness_normalized": im_data["im_roughness_normalized"],
                "dominant_im_type"       : im_data["dominant_im_type"],
                "audible_count"          : im_data["audible_count"],
                "aligned_count"          : im_data["aligned_count"],
                "clashing_count"         : im_data["clashing_count"],
                "im_type_counts"         : im_data["im_type_counts"],
                "worst_offenders"        : im_data["worst_offenders"],
            },
        },
        "level_2_brainstem": {
        ...
        "tension_breakdown"       : {
            ...
            "im_component"         : round(t_im,    4),
            "weights"              : {
                "roughness": 0.35, "harmonicity": 0.25,
                "tonal"    : 0.25, "intermodulation": 0.15},
        },
    }


# ──────────────────────────────────────────────────────────────────────────────
# MODULE 0.11  —  INTERMODULATION / COMBINATION-TONE SOLVER
# ──────────────────────────────────────────────────────────────────────────────
...
def intermodulation_products(
        ...
        max_order      : int   = 3,
        level_db       : float = 70.0,
        min_audible_db : float = -28.0,
) -> List[Dict[str, Any]]:
    """
    Compute psychoacoustically significant intermodulation (IM) combination tones
    for a multi-partial spectrum.

    Physical basis
    ──────────────
    The IHC / basilar-membrane transfer function is mildly compressive — its
    Taylor expansion contains quadratic (a₂) and cubic (a₃) nonlinearity terms.
    When two sinusoids at f₁ < f₂ enter the cochlea simultaneously they generate:

      Order 2 (quadratic distortion):
        QDT  : f₂ − f₁            (difference tone; Tartini 1754)
        SUM₂ : f₁ + f₂            (summation tone; usually masked)

      Order 3 (cubic distortion, dominant in normal hearing):
        CDT_lo : 2f₁ − f₂         (cubic difference tone — most audible IM product;
                                    corresponds to DPOAE at 2f₁ − f₂)
        CDT_hi : 2f₂ − f₁         (upper cubic, usually masked by primaries)

    Amplitude model  (Goldstein 1967; Smoorenburg 1972; Zwicker & Fastl 1999)
    ──────────────────────────────────────────────────────────────────────────
    Empirical coefficients calibrated against published DPOAE growth functions:
        QDT  amplitude  ≈ γ₂ · A₁ · A₂             γ₂ = 0.08
        SUM₂ amplitude  ≈ γ₂ · A₁ · A₂ · 0.5
        CDT_lo amplitude ≈ γ₃ · A₁² · A₂            γ₃ = 0.25
        CDT_hi amplitude ≈ γ₃ · A₁ · A₂²

    A product is reported only if:
      (a) frequency > 20 Hz
      (b) estimated level ≥ min_audible_db re loudest primary
      (c) NOT within 1 Hz of any existing primary partial

    Returns (list sorted by frequency)
    ────────────────────────────────────────────────────────────────────────
    frequency_hz, order, type, parent_freqs, amplitude, level_db_rel, audible
    """
    amp_ref  = max(amps) if amps else 1.0
    n        = len(freqs)
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            a1 = amps[i] / (amp_ref + 1e-12)
            a2 = amps[j] / (amp_ref + 1e-12)
            if f1 > f2:
                f1, f2 = f2, f1
                a1, a2 = a2, a1

            combos = [
                (f2 - f1,         2, "QDT",    0.08  * a1 * a2),
                (f1 + f2,         2, "SUM",    0.04  * a1 * a2),
            ]
            if max_order >= 3:
                combos += [
                    (2.0*f1 - f2, 3, "CDT_lo", 0.25 * (a1**2) * a2),
                    (2.0*f2 - f1, 3, "CDT_hi", 0.25 * a1 * (a2**2)),
                ]

            for f_im, order, im_type, amp_im in combos:
                if f_im < 20.0:
                    continue
                level_rel = 20.0 * math.log10(amp_im + 1e-12)
                if level_rel < min_audible_db:
                    continue
                if any(abs(f_im - fp) < 1.0 for fp in freqs):
                    continue
                products.append({
                    "frequency_hz" : round(f_im, 2),
                    "order"        : order,
                    "type"         : im_type,
                    "parent_freqs" : [round(f1, 2), round(f2, 2)],
                    "amplitude"    : round(amp_im, 6),
                    "level_db_rel" : round(level_rel, 1),
                    "audible"      : level_rel > -20.0,
                })

    # Deduplicate: merge products within 2 Hz, keep highest amplitude
    products.sort(key=lambda x: -x["amplitude"])
    deduped : List[Dict[str, Any]] = []
    seen_f  : List[float] = []
    for p in products:
        f = p["frequency_hz"]
        if not any(abs(f - fs) < 2.0 for fs in seen_f):
            deduped.append(p)
            seen_f.append(f)

    return sorted(deduped, key=lambda x: x["frequency_hz"])


def im_consonance_index(
        im_products     : List[Dict[str, Any]],
    ...
    """
    Intermodulation Consonance Index (ICI) — octave-invariant version.

    Measures how well audible IM products align with the chord's own pitches,
    assessed in **pitch-class space** (octave-equivalent).  This is the correct
    perceptual frame: a CDT landing on G3 is just as "aligned" with a chord
    containing G4 as if it had landed at G4 exactly.

    • High ICI (→ 1): IM products land on chord pitch classes → reinforce the
      chord's harmonic identity, minimal net roughness.
    • Low  ICI (→ 0): IM products fall between pitch classes → phantom partials
      beat against primaries, increasing perceived roughness.

    Algorithm
    ─────────
    For each audible IM product at f_im:
      1. Convert f_im to a fractional pitch class in the given EDO.
      2. Compute the minimum distance (in cents) from any chord pitch class,
         wrapping at the octave → this is the octave-invariant deviation.
      3. Aligned if min_deviation ≤ tolerance_cents.
      4. If clashing, compute P-L roughness between the IM product and the
         nearest chord tone (in Hz) in the voiced register.

    ICI = Σ(amp_i · aligned_i) / Σ(amp_i)   (amplitude-weighted)

    Parameters
    ──────────
    im_products     : output from intermodulation_products()
    chord_freqs     : fundamental Hz of voiced chord notes (for roughness calc)
    tolerance_cents : alignment window in cents (default 30¢ ≈ 1/4 semitone)
    chord_pcs       : integer pitch classes [0, edo); if None, derived from freqs
    c4_hz           : reference pitch (261.625 Hz)
    edo             : divisions of the octave

    Returns
    ───────
    ici, ici_label, im_roughness, aligned_count, clashing_count,
    audible_count, details (per-product breakdown)
    """
    audible = [p for p in im_products if p["audible"]]
    if not audible or not chord_freqs:
        return {"ici": 1.0, "ici_label": "reinforcing",
                "im_roughness": 0.0, "aligned_count": 0,
                "clashing_count": 0, "audible_count": 0, "details": []}

    # Build chord pitch classes if not supplied
    if chord_pcs is None:
        chord_pcs = []
        for f in chord_freqs:
            if f > 0:
                # fractional pc: log2(f/c4_hz) * edo — wrap to [0, edo)
                frac_pc = (math.log2(f / c4_hz) * edo) % edo
                chord_pcs.append(frac_pc)

    # Step size in cents (for EDO-invariant comparison)
    cents_per_step = 1200.0 / edo

    def _pc_deviation_cents(f_im: float) -> float:
        """Minimum cents deviation of f_im from any chord pitch class (octave-invariant)."""
        if f_im <= 0:
            return 600.0
        frac_pc = (math.log2(f_im / c4_hz) * edo) % edo
        min_dev = float('inf')
        for cpc in chord_pcs:
            # Circular distance on the EDO ring
            d_steps = abs(frac_pc - cpc) % edo
            d_steps = min(d_steps, edo - d_steps)
            min_dev = min(min_dev, d_steps * cents_per_step)
        return min_dev

    total_amp    = sum(p["amplitude"] for p in audible) + 1e-12
    aligned_amp  = 0.0
    im_roughness = 0.0
    n_aligned    = 0
    n_clashing   = 0
    ...
    for p in audible:
        f_im  = p["frequency_hz"]
        amp   = p["amplitude"]
        dev_cents  = _pc_deviation_cents(f_im)
        is_aligned = dev_cents <= tolerance_cents

        if is_aligned:
            aligned_amp += amp
            n_aligned   += 1
        else:
            # Closest chord tone in voiced Hz space for roughness calculation
            nearest_f = min(chord_freqs,
                            key=lambda f: abs(math.log2(max(f, 1e-6) / max(f_im, 1e-6))))
            im_roughness += plomp_levelt_roughness_pair(f_im, nearest_f, amp, 0.4)
            n_clashing   += 1

        details.append({
            "im_freq_hz"       : round(f_im, 2),
            "type"             : p["type"],
            "order"            : p["order"],
            "pc_deviation_cents": round(dev_cents, 1),
            "aligned"          : is_aligned,
            "amplitude"        : round(amp, 6),
            "level_db_rel"     : p["level_db_rel"],
        })

    ici = aligned_amp / total_amp
    label = ("reinforcing" if ici > 0.60 else
             "mixed"       if ici > 0.30 else "dissonant")

    return {
        "ici"           : round(ici, 4),
        "ici_label"     : label,
        "im_roughness"  : round(im_roughness, 6),
        "aligned_count" : n_aligned,
        "clashing_count": n_clashing,
        "audible_count" : len(audible),
        "details"       : details,
    }


def chord_intermodulation_analysis(
        pitch_classes : List[int],
        c4_hz         : float = 261.625565,
        octave        : int   = 4,
        n_harmonics   : int   = 8,
    ...
    """
    Full intermodulation analysis for a chord.

    Builds IHC-compressed harmonic partial series (close-position voicing,
    same convention as chord_roughness_psychoacoustic), then:

      1. Calls intermodulation_products() on the full partial spectrum.
      2. Evaluates im_consonance_index() against chord fundamentals.
      3. Summarises perceptual impact.

    Calibration reference (im_roughness_normalized)
    ────────────────────────────────────────────────
      Major triad  C-E-G          ≈ 0.04–0.10  (reinforcing)
      Minor 7th    C-Eb-G-Bb      ≈ 0.12–0.20  (mixed)
      Minor 2nd    C-Db           ≈ 0.30–0.45  (dissonant)

    Returns
    ───────
    im_products, audible_products, ici, ici_label, im_roughness,
    im_roughness_normalized, dominant_im_type, worst_offenders,
    note_frequencies_hz, im_type_counts, aligned_count,
    clashing_count, audible_count
    """
    if not pitch_classes:
        return {
            "im_products": [], "audible_products": [], "ici": 1.0,
            "ici_label": "reinforcing", "im_roughness": 0.0,
            "im_roughness_normalized": 0.0, "dominant_im_type": "none",
            "worst_offenders": [], "note_frequencies_hz": [],
            "im_type_counts": {}, "aligned_count": 0,
            "clashing_count": 0, "audible_count": 0,
        }
    ...
    # ── Two-tier amplitude model ───────────────────────────────────────────
    # Physical justification:
    #   1. Basilar-membrane (BM) nonlinearity that GENERATES combination tones
    #      operates on the pre-compression mechanical signal. The natural
    #      harmonic rolloff (≈ 0.88^k per octave) governs which IM products are
    #      strongest — lower harmonics produce much stronger CDTs.
    #   2. IHC compression (4:1 above 40 dB SPL) operates AFTER BM mechanics.
    #      Using post-compression amplitudes for IM generation would falsely
    #      equalize all harmonics and inflate the count of weak-harmonic IM
    #      products to the same level as fundamental-pair products.
    #
    # Solution: use PRE-compression rolloff amplitudes for IM product amplitude
    # estimation (reflecting BM nonlinearity strength), and normalise with
    # amplitude² weighting in ICI to further down-weight upper harmonics.

    all_freqs_im : List[float] = []
    all_amps_im  : List[float] = []   # pre-compression, for IM generation
    for f0 in note_freqs:
        fs, as_ = _build_harmonic_series(f0, n_harmonics, rolloff)
        all_freqs_im.extend(fs)
        all_amps_im.extend(as_)

    # Normalise pre-compression amplitudes globally (max=1)
    max_a_im    = max(all_amps_im) if all_amps_im else 1.0
    all_amps_im = [a / max_a_im for a in all_amps_im]

    im_products = intermodulation_products(
        all_freqs_im, all_amps_im, max_order=max_order, level_db=level_db)
    ici_data    = im_consonance_index(
        im_products, note_freqs,
        chord_pcs=[p % edo for p in pcs],
        c4_hz=c4_hz, edo=edo)
    audible     = [p for p in im_products if p["audible"]]
    ...
    offenders = sorted(
        [d for d in ici_data["details"] if not d["aligned"]],
        key=lambda d: d["pc_deviation_cents"], reverse=True,
    )[:3]

    # Normalise: calibrated so m2 cluster IM roughness ≈ 1.0
    _IM_ROUGH_REF = 0.40
    im_rough_norm = round(min(1.0, ici_data["im_roughness"] / _IM_ROUGH_REF), 4)

    return {
        "im_products"            : im_products,
        "audible_products"       : audible,
        "ici"                    : ici_data["ici"],
        "ici_label"              : ici_data["ici_label"],
        "im_roughness"           : ici_data["im_roughness"],
        "im_roughness_normalized": im_rough_norm,
        "dominant_im_type"       : dominant,
        "worst_offenders"        : offenders,
        "note_frequencies_hz"    : [round(f, 2) for f in note_freqs],
        "im_type_counts"         : type_counts,
        "aligned_count"          : ici_data["aligned_count"],
        "clashing_count"         : ici_data["clashing_count"],
        "audible_count"          : ici_data["audible_count"],
    }
...
def suggest_completion(pitch_classes: List[int], key: int,
    ...
    psychoacoustic model including the intermodulation solver.

    Scoring (additive, all normalized 0–1, higher = better):
        35% — spectral consonance          (1 − Sethares roughness_normalized)
        22% — tonal stability              (Krumhansl–Kessler KK profile)
        18% — harmonicity                  (Terhardt virtual pitch strength)
        13% — voice-leading smoothness     (step motion preference)
        12% — IM consonance index (ICI)    (1 − im_roughness_normalized;
                                            penalises choices that generate
                                            clashing combination tones)

    The ICI component specifically captures dissonance that Sethares roughness
    misses: two notes may have low pairwise roughness but their combination
    tones (CDTs at 2f₁−f₂) may land in dissonant positions relative to the
    rest of the chord.
    ...
    for pc in range(edo):
        ...
        im_data_cand   = chord_intermodulation_analysis(test_pcs, c4_hz, edo=edo)

        consonance   = roughness_data["consonance_score"]
        kk_stability = kk_data["kk_normalized"]
        harmonicity  = vp_data["harmonicity"]
        im_score     = 1.0 - im_data_cand["im_roughness_normalized"]  # higher = better
        ...
        if im_data_cand["ici_label"] == "reinforcing":
            reasons.append(
                f"IM consonant: {im_data_cand['aligned_count']} combination tones "
                f"align with chord tones (ICI={im_data_cand['ici']:.2f})")
        elif im_data_cand["ici_label"] == "dissonant":
            reasons.append(
                f"IM warning: {im_data_cand['clashing_count']} clashing combination "
                f"tones (ICI={im_data_cand['ici']:.2f})")
        ...
        suggestions.append({
            ...
            "im_consonance_index"     : im_data_cand["ici"],
            "im_ici_label"            : im_data_cand["ici_label"],
            "im_roughness_normalized" : im_data_cand["im_roughness_normalized"],
            "im_dominant_type"        : im_data_cand["dominant_im_type"],
            # Context
...
def handle(cmd: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if c == "analyze_chord":
        ...
        # Append intermodulation summary (lightweight: no full product list)
        im_chord = chord_intermodulation_analysis(pcs, c4hz, edo=edo)
        res["intermodulation"] = {
            "ici"                    : im_chord["ici"],
            "ici_label"              : im_chord["ici_label"],
            "im_roughness_normalized": im_chord["im_roughness_normalized"],
            "dominant_im_type"       : im_chord["dominant_im_type"],
            "audible_count"          : im_chord["audible_count"],
            "clashing_count"         : im_chord["clashing_count"],
            "worst_offenders"        : im_chord["worst_offenders"],
        }
        return res

    elif c == "intermodulation_analysis":
        """
        Full intermodulation / combination-tone analysis for a chord.

        Computes all pairwise QDT / CDT_lo / CDT_hi / SUM products from the
        chord's IHC-compressed harmonic spectrum, then evaluates:
          • Intermodulation Consonance Index (ICI) — alignment with chord tones
          • IM roughness — P-L roughness from clashing phantom partials
          • Product catalogue: frequency, type, amplitude, audibility

        Accepts: pcs, c4_hz, octave (default 4), n_harmonics (default 8),
                 rolloff (default 0.88), level_db (default 70),
                 max_order (default 3; set 2 for quadratic only)
        """
        if not pcs: pcs = triad_pcs(root, qual, edo)
        im_full = chord_intermodulation_analysis(
            pcs, c4hz,
            octave      = cmd.get("octave", 4),
            n_harmonics = cmd.get("n_harmonics", 8),
            rolloff     = cmd.get("rolloff", 0.88),
            level_db    = cmd.get("level_db", 70.0),
            max_order   = cmd.get("max_order", 3),
            edo         = edo,
        )
        return {"result": "intermodulation_analysis", **im_full}
        ...
        return {
            ...
            "modules": [
                "psychoacoustic_engine",       # 0.1–0.10: peripheral/brainstem/cortical
                "intermodulation_solver",      # 0.11: CDT/QDT combination-tone analysis
                "set_class",
                "plr_neo_riemannian",
                "functional_analysis",
                "voice_leading_orbifold",
                "tonnetz_tension_hybrid",
                "pattern_recognition",
                "edo_ji_lattice",
                "voice_completion_psychoacoustic",  # now includes ICI scoring
                "pivot_search",
            ],
            "commands": [
                # Psychoacoustic engine (MODULE 0)
                "psychoacoustic_analysis",   # full 4-level model incl. IM
                "spectral_roughness",
                "virtual_pitch",
                "masking_analysis",
                "roughness_curve",
                "tonal_hierarchy",
                # Intermodulation solver (MODULE 0.11)
                "intermodulation_analysis",  # keys: pcs, octave, n_harmonics, max_order
                #   → im_products, audible_products, ici, ici_label,
                #     im_roughness_normalized, worst_offenders, dominant_im_type
                # Chord analysis
                "analyze_chord",        # alias: functional_analysis, analyze
                #   → now includes intermodulation summary block
                ...
                "tonnetz_projection",
                # Harmony / completion
                "suggest_completion",   # alias: voice_completion, chord_completion
                #   → now includes im_consonance_index (ICI) in per-candidate scoring
                "modal_mixture",
                # Pattern recognition
...
def main():
    sys.stderr.write(
        ...
        "  IM Solver  : CDT/QDT combination tones (Goldstein/Smoorenburg) + ICI scoring\n"
        "  Brainstem  : Terhardt virtual pitch (SHS) + AN rate-place profile\n"
        "  Cortical   : Krumhansl–Kessler tonal hierarchy + Tonnetz hybrid\n"
        "  Tension    : 4-component model (roughness+harmonicity+tonal+IM)\n"
    )
    sys.stderr.flush()
