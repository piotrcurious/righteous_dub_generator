...
def chord_roughness_psychoacoustic(
    ...
    Takes raw frequencies in Hz.

    Normalization: roughness is divided by the roughness of a reference dyad
    (one EDO-step apart at the same register) scaled by the number of note pairs,
    ...
    for ni in range(len(note_freqs)):
        for nj in range(ni + 1, len(note_freqs)):
            ...
            pair_roughness.append({
                "freq_a": round(note_freqs[ni], 2), "freq_b": round(note_freqs[nj], 2),
...
def virtual_pitch_strength(
    ...
    # Convert best F₀ to pitch class (relative to C4=261.63)
    best_pc = int(round(math.log2(best_f0 / 261.63) * edo)) % edo
    ...
    top3_out = []
    for f, s in top3:
        pc_candidate = int(round(math.log2(f / 261.63) * edo)) % edo
...
def chord_masking_analysis(freqs       : List[float],
                           level_db    : float = 70.0,
    ...
    audibility = []
    for i, (pc, f_probe) in enumerate(zip(pcs, note_freqs)):
    ...
    for i in range(len(note_freqs)):
        for j in range(i + 1, len(note_freqs)):
            cb_lo  = critical_bandwidth_zwicker(note_freqs[i])
            cb_hi  = critical_bandwidth_zwicker(note_freqs[j])
            delta  = abs(note_freqs[j] - note_freqs[i])
            avg_cb = 0.5 * (cb_lo + cb_hi)
            overlap = max(0.0, 1.0 - delta / avg_cb)   # 1 = full overlap, 0 = no
            bm_dist = tonotopic_distance_mm(note_freqs[i], note_freqs[j])
            cb_overlaps.append({
                "freq_a": round(note_freqs[i], 2), "freq_b": round(note_freqs[j], 2),
...
def tonal_hierarchy_score(pitch_classes: List[int], key: int,
    ...
    if edo == 12:
        ...
        # Map back to 12-EDO for KK profiles
        mapped_pcs = [int(round(pc * 12 / edo)) % 12 for pc in pcs]
        mapped_key = int(round(key * 12 / edo)) % 12
        raw = sum(profile[(pc - mapped_key) % 12] for pc in mapped_pcs) / len(pcs)
...
def auditory_cortex_model(
    ...
    # ── Level 1: Peripheral ─────────────────────────────────────────────
    roughness_data = chord_roughness_psychoacoustic(note_freqs, n_harmonics, edo=edo)
    masking_data   = chord_masking_analysis(note_freqs, level_db, edo=edo)

    # ── Level 2: Brainstem ──────────────────────────────────────────────
    vp_data = virtual_pitch_strength(note_freqs, n_harmonics, edo=edo)
    ...
    centroid_hz = sum(note_freqs) / len(note_freqs) if note_freqs else 261.63
    centroid_pc = int(round(math.log2(centroid_hz / 261.63) * edo)) % edo
...
def suggest_completion(pitch_classes: List[int], key: int,
    ...
    if edo == 12:
        ...
        diat = set((key + int(round(s * edo / 12))) % edo for s in [0,2,4,5,7,9,11])

    ji_map = {0:0.0, 2:203.9, 4:386.3, 5:498.0, 7:702.0, 9:884.4, 11:1088.3,
              3:315.6, 6:590.2, 8:813.7, 10:969.0, 1:111.7}

    suggestions = []
    # Only suggest the 12 primary degrees of the EDO
    for i in range(12):
        pc = int(round(i * edo / 12)) % edo
        if pc in existing:
            continue
        test_pcs = sorted(existing | {pc})

        # ── Psychoacoustic scoring ────────────────────────────────────
        test_freqs     = [261.63 * 2.0 ** (p / edo) for p in test_pcs]
        roughness_data = chord_roughness_psychoacoustic(test_freqs, edo=edo)
        kk_data        = tonal_hierarchy_score(test_pcs, key, edo=edo)
        vp_data        = virtual_pitch_strength(test_freqs, edo=edo)
...
def handle(cmd: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # If freqs provided but no pcs, derive pcs
    if freqs and not pcs:
        pcs = [int(round(math.log2(f / 261.63) * edo)) % edo for f in freqs]

    # If pcs provided but no freqs, derive freqs (assume octave 4 for analysis)
    if pcs and not freqs:
        freqs = [261.63 * 2.0 ** (p / edo) for p in pcs]
    ...
    if c == "analyze_chord":
        ...
        return {
            ...
            **virtual_pitch_strength(freqs, cmd.get("n_harmonics", 8), edo)
        }
        ...
        return {
            ...
            **chord_masking_analysis(freqs, cmd.get("level_db", 70.0), edo)
        }
        ...
        return {
            ...
            **tonal_hierarchy_score(pcs, key, cmd.get("mode", "major"), edo)
        }
